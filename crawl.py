...
# ---- 収集件数（お好みで調整）----
LIMITS = {
    "リュウジ": 600,   # wp-json から新着順（過去分まで広く収集）
...
EXCLUDE_HARD = [
    "ほたて", "帆立", "ホタテ",
    "イカ", "烏賊", "するめ",  # ひらがな「いか」は「美味しいから」等に誤爆するため不可
    "チンゲン", "青梗菜",
    "きのこ", "キノコ", "しめじ", "シメジ", "えのき", "エノキ",
    "舞茸", "まいたけ", "マイタケ", "エリンギ", "しいたけ", "椎茸",
    "マッシュルーム", "なめこ", "ヒラタケ",
]
# コスト/用途で除外（牛肉・魚介）。※15人分想定のため。個人で許容するなら空にしてOK
# ※ 部分一致の誤爆を避け、漢字/カタカナ主体の安全な表記のみ使用
#   （牛→牛乳, たら→炒めたら, かに→確かに, さけ→避ける 等の誤除外を防止）
EXCLUDE_COST = [
    "牛肉", "牛バラ", "牛こま", "牛ロース", "牛ステーキ", "牛すじ", "ビーフ",
    "鮭", "サーモン", "サバ", "鯖", "ブリ", "鰤",
    "マグロ", "鮪", "鯵", "イワシ", "鰯", "タラ", "鱈", "サンマ", "秋刀魚", "カツオ", "鰹",
    "エビ", "海老", "カニ", "蟹", "タコ", "蛸",
    "あさり", "しじみ", "白身魚", "刺身", "切り身", "海鮮", "魚介",
]
